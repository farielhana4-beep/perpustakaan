...
export default function AuthLayout({ children }) {
  const { flash = {}, locale = 'id', availableLocales = [], locales = [] } = usePage().props
      ...
      <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center">
        <div className="w-full rounded-3xl border border-slate-200 bg-white p-8 shadow-[0_30px_80px_rgba(15,23,42,0.10)]">
          <div className="flex items-center justify-between gap-4">
            <BrandMark compact />
            <LocaleSwitcher
              className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold tracking-[0.2em] text-slate-700 shadow-sm outline-none transition focus:border-sky-400 focus:ring-4 focus:ring-sky-100"
              locale={locale}
              availableLocales={switcherLocales}
            />
          </div>
