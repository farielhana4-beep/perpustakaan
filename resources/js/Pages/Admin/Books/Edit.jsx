...
export default function Edit({ book, categories }) {
  ...
  return (
    <AdminLayout>
      <Head title={`${t?.books?.edit_title} ${book.title}`} />
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-bold text-slate-900">{t?.books?.edit_title}</h1>
          </div>
        ...
        <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="grid gap-4 md:grid-cols-[140px_1fr] md:items-start">
            ...
            <div className="space-y-4">
              <div>
                <input type="text" value={data.title} onChange={(e) => setData('title', e.target.value)} className="input" name="title" />
                {errors.title && <div className="mt-2 text-sm text-red-500">{errors.title}</div>}
              </div>

              <div>
                <input type="text" value={data.author} onChange={(e) => setData('author', e.target.value)} className="input" name="author" />
                {errors.author && <div className="mt-2 text-sm text-red-500">{errors.author}</div>}
              </div>

              <div>
                <input type="text" value={data.isbn} onChange={(e) => setData('isbn', e.target.value)} className="input" name="isbn" />
                {errors.isbn && <div className="mt-2 text-sm text-red-500">{errors.isbn}</div>}
              </div>

              <div>
                <select value={data.category_id} onChange={(e) => setData('category_id', e.target.value)} className="input" name="category_id">
              ...
              <div>
                <input type="number" value={data.stock} onChange={(e) => setData('stock', e.target.value)} className="input" name="stock" />
                {errors.stock && <div className="mt-2 text-sm text-red-500">{errors.stock}</div>}
              </div>

              <div>
                <input type="file" onChange={(e) => setData('image', e.target.files[0])} className="input" name="image" />
                {errors.image && <div className="mt-2 text-sm text-red-500">{errors.image}</div>}
              </div>
            </div>
          </div>

          <button type="submit" disabled={processing} className="rounded-xl bg-sky-500 px-4 py-2 font-semibold text-white transition hover:bg-sky-600 disabled:cursor-not-allowed disabled:opacity-60">
            {t?.books?.update_button}
          </button>
        </form>
      </div>
