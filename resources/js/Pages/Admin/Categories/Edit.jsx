...
export default function Edit({ category }) {
  ...
  return (
    <AdminLayout>
      <Head title={`${t?.categories?.edit_title} ${category.name}`} />
      <SimpleForm title={t?.categories?.edit_title} href="/admin/categories" buttonLabel={t?.categories?.update_button} onSubmit={submit} processing={processing} flash={flash} errors={errors} data={data} setData={setData} />
    </AdminLayout>
  )
}
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-sky-600">{t?.categories?.title}</p>
      ...
      <form onSubmit={onSubmit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <Field label={t?.form?.slug} error={errors.slug}>
        ...
        <button type="submit" disabled={processing} className="mt-6 rounded-xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:opacity-60">
          {processing ? t?.common?.updating : buttonLabel}
        </button>
      </form>
