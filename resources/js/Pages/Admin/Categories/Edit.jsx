...
export default function Edit({ category }) {
  ...
  return (
    <AdminLayout>
      ...
      <SimpleForm title="Edit Category" href="/admin/categories" buttonLabel="Update Category" onSubmit={submit} processing={processing} flash={flash} errors={errors} data={data} setData={setData} />
    </AdminLayout>
  )
}
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <form onSubmit={onSubmit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <button type="submit" disabled={processing} className="mt-6 rounded-xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-sky-700 disabled:opacity-60">{processing ? 'Updating...' : buttonLabel}</button>
      </form>
