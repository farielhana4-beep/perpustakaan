    ...
    <AdminLayout>
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-sky-600">{t?.settings?.title}</p>
          <h1 className="mt-2 text-3xl font-bold text-slate-900">{t?.settings?.library_settings}</h1>
          <p className="mt-2 text-sm text-slate-600">{t?.settings?.configure_rules}</p>
        </section>

        <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="grid gap-5">
            <Field label={t?.form?.fine_per_day} error={errors.fine_per_day}>
              <input className="input" type="number" min="0" value={data.fine_per_day} onChange={(e) => setData('fine_per_day', e.target.value)} />
            </Field>
            <Field label={t?.form?.max_borrow_days} error={errors.max_borrow_days}>
              <input className="input" type="number" min="1" value={data.max_borrow_days} onChange={(e) => setData('max_borrow_days', e.target.value)} />
            </Field>
            ...
            <Field label={t?.form?.max_books_per_user} error={errors.max_books_per_user}>
              <input className="input" type="number" min="1" value={data.max_books_per_user} onChange={(e) => setData('max_books_per_user', e.target.value)} />
            </Field>
          </div>
